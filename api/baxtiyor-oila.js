...
// islom.uz dan Baxtiyor Oila maqolalarini scraping qilish
async function scrapeBaxtiyorOilaArticles(categoryNum) {
  try {
    const url = `https://islom.uz/maqolalar/107/${categoryNum}`;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (category) {
      ...
      if (categoryNum < 1 || categoryNum > 15) {
        return res.status(400).json({
          success: false,
          error: 'Invalid category number (1-15)'
        });
      }
      
      const cacheKey = `baxtiyor_oila_cat_${categoryNum}`;
      ...
      try {
        const articles = await scrapeBaxtiyorOilaArticles(categoryNum);
    ...
    const cacheKey = 'baxtiyor_oila_quick';
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      try {
        const articles = await scrapeBaxtiyorOilaArticles(i);
    ...
    return res.status(200).json({
      ...
      articles: [
        {
          id: 1,
          title: 'Baxtiyor oila haqida',
          intro: 'Baxtiyor oila barpo etish har bir musulmonning orzu-istagidir.',
          image: '',
          link: 'https://islom.uz',
          category: 1
        },
        {
          ...
          intro: 'Oilaviy munosabatlarda hurmat va muhabbat asosiy omillardir.',
          image: '',
          link: 'https://islom.uz',
          category: 1
        }
