...
const BaxtiyorOila = () => {
  ...
  const totalCategories = 15; // Jami kategoriyalar

  // Kategoriya batch ni aniqlash (1-3, 4-6, 7-9, 10-12, 13-15)
  ...
  const fetchCategories = async (startCategory, endCategory) => {
    ...
    try {
      ...
      for (const categoryNum of categoriesToLoad) {
        const response = await fetch(`/api/baxtiyor-oila?category=${categoryNum}`);
  ...
  const fetchArticleContent = async (articleUrl) => {
    try {
      ...
      const response = await fetch(`/api/baxtiyor-oila?articleUrl=${encodeURIComponent(articleUrl)}`);
      const data = await response.json();
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          ...
          <Text style={{ color: 'rgba(255,255,255,0.9)', fontSize: '18px' }}>
            Islomda oila qurish va baxtiyor hayot kechirish haqida maqolalar
          </Text>
        </div>
